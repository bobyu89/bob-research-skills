...
Signal:
- `n` is reported as questionnaire items, log events, chat turns, think-aloud segments, sessions, or repeated readings.
- The independent unit is likely the participant, the ward or class, or the DBR iteration cohort.

Risk:
- The test may overstate precision and produce artificially small p values.

Fix:
- Analyse independent units, aggregate within-participant subsamples, or use a hierarchical / mixed-effects / GEE model when justified.
- Show subsample-level spread visually without treating every subsample as independent.

### Uncorrected multiple comparisons

Signal:
- Many scale subscales, items, time points, groups, pairwise contrasts, or exploratory endpoints are tested.
...
Signal:
- Association is written as mechanism, prediction, or causality without design support; usability or satisfaction scores are written as learning outcomes.

Risk:
- Confounding, non-independence, and model extrapolation may be ignored.

Fix:
- Reword as association unless experimental or causal identification supports stronger claims. Keep perceived usability (SUS) separate from competence outcomes.
...
Fix:
- State SD, SE, confidence interval, interquartile range, or other convention (APA abbreviations, italic).
