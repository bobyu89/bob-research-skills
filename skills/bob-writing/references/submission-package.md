...
9. Templates (Markdown/Word default; LaTeX optional)
10. Output format
...
| Graphical abstract | required / optional / N/A | route to bob-figure | status |
...
Default deliverable is Word or plain text. Use `templates/submission/cover-letter.md` (English and Traditional Chinese versions, nursing-journal context) and paste into Word. The LaTeX templates below are optional and only used when the user asks for `.tex` files; use them only for initial submission:
...
- Never reuse the `bob-response` revision cover-letter template; it is explicitly for a revised manuscript.
