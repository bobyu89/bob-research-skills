...
> `bob-polishing/references/published-article-patterns.md`. This file adds
...
- Make the **choice of system/problem explicit** — *"We chose this model system because…"* — this satisfies the "科學問題要科學" expectation: the question must be motivated, not assumed.
...
- 保留資訊鏈:`現象（現在式）→ 然而/目前仍/尚不清楚 → 本文提出 X → 帶一個具體數字的結果 → 意義昇華`。
- 標題按體裁切模板:方法文用名詞短語、賣點文用陳述句、工具用"系統名:功能"、論辯文可用疑問句;**標題不放數字與結論**。
- gap 必帶訊號詞(然而/不幸的是/仍是挑戰/鮮有研究/缺乏);高階寫法用數字量化稀缺。
- 貢獻句明確給出選題理由("之所以選該體系,是因為…"),呼應"科學問題要科學"。
- 對沖詞(可能/提示/很可能)只用於意義句;少用"顯著"——對應英文 `significantly` 在本語料 0 次,改用"明顯/大幅/相當"(notably/substantially/considerably)。
