> 非預設：本片段由上游保留，未經本集驗證。只在使用者明確指定 Nature 系列期刊時載入；所有數字以期刊官網為準並註明查核日期。共用層不含 Nature 專屬格式檔。

# Journal: Nature Machine Intelligence — writing

## Read the shared contract first

The shared layer does not carry an NMI format file. Check the journal's
current author instructions for content types, word/display limits, initial
files, code/data policy and production rules, and record the verification
date. Treat every number below as an example.

The notes below are the **drafting action layer** on top of those facts.

For Results or Discussion drafting, also open
`../../../../bob-shared/core/results-discussion-escalation.md`. It records
corpus-derived Nature-style writing patterns, not official submission
requirements.

For Introduction drafting, also open
`../../../../bob-shared/core/introduction-funnel.md`. It records
corpus-derived Nature-style writing patterns, not official submission
requirements.

For abstract drafting, also open
`../../../../bob-shared/core/abstract-evidence-chain.md`. It records corpus-derived
