...
`bob-response` is evaluated with synthetic Markdown fixtures. These tests are not executable
...
- Traditional Chinese author-note workflows produce usable English response drafts and 中文核對 notes;
- at least one thesis committee response table (`committee-response` mode) is reviewed by the author;
