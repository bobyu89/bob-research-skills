...
9. Chinese confirmation notes (中文核對) when the user writes in Chinese.

For `committee-response` mode, replace items 1 to 6 with the single committee response table
described in `references/thesis-committee-response.md`.
...
When modifying manuscript text, work on a copy of the original manuscript and mark changed passages in red. For LaTeX manuscripts, wrap changed text in `\revised{...}` from `templates/revised-manuscript-redline.tex`. For Word manuscripts, use tracked changes or red text (underline when the journal asks for it) and keep a clean version separate from the marked copy.
...
## Markdown and Word deliverables

The default deliverable is Markdown, converted to Word by the `docx` skill when the user needs a
file. Use `templates/response-letter.md` (one copy per reviewer; English and Traditional Chinese
versions) and `templates/revision-cover-letter.md`. For thesis committee tables use
`templates/committee-response-table.md`.

## LaTeX deliverables (optional)

When the user explicitly asks for LaTeX, use the templates in `templates/`:
