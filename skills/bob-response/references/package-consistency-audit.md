...
audited before submission. The script in section 1 supports LaTeX packages only; for Word packages,
perform the same checks manually (search the marked manuscript for every quoted passage, count
comment/response pairs in the table, and compare the clean and marked copies with Word's Compare
function). It covers the failures that are **mechanically verifiable** and that
...
sweep from `../bob-shared/core/consistency-sweep.md`: headline counts that do not reconcile with the
