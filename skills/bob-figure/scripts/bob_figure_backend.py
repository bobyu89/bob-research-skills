#!/usr/bin/env python3
"""Read or write the user's default bob-figure plotting backend (Python or R)."""
...
def config_path() -> Path:
    override = os.environ.get("BOB_FIGURE_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.config/bob-research-skills/bob-figure.json").expanduser()
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Invalid bob-figure config JSON at {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise SystemExit(f"Invalid bob-figure config at {path}: expected object")
    return data
