...
Prefer **unified method families across all panels** over maximal hue separation. For dense multi-panel pages, use the low-saturation pastel family described in `references/api.md` (kept under its upstream name there) and reserve green/red mainly for gains, drops, and other directional cues. Every palette must remain colour-blind friendly and legible in greyscale print; the `thesis-word` target additionally requires monochrome readability.
...
- Only when the user explicitly targets a Nature-family journal, read `references/nature-2026-observations.md` before choosing layout; it is not a default for nursing, medical-education, or digital-health venues.
...
- Python or R figures for **journal manuscripts, a master's thesis in Word, or research reports** in nursing, medical education, or digital health (and any other peer-reviewed venue).
- Requests involving **grouped bars, trend lines, heatmaps, radar plots, multi-panel grids**, or **PDF/SVG/high-DPI** output.
- Any mention of "publication figure", "paper figure", "thesis figure", "figures4papers", "scientific-figure-making", "R plotting template", or "high-quality scientific plot".
