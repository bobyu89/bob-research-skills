...
Use this reference to build an internal concern ledger after the five review axes have been assessed. It is a coverage and traceability aid, not an official journal taxonomy, reviewer-persona selector, or historical-frequency model.
...
| `clinical-validity` | Cohort definition, endpoint choice, external validation, calibration, clinical or educational utility, and generalizability support clinical or educational claims. |
| `ethical-governance` | IRB approval, consent, privacy and personal-data protection, registration, permissions for instruments, generative-AI disclosure, and responsible data handling are reported when required. |
...
| `writing-clarity` | The argument, terminology, abstract/body consistency, and explanation for adjacent-field readers make the evidence chain understandable. |
...
## Mapping to the five review axes

Each internal axis rolls up to one user-visible review axis for the `Axis` field of a concern:

| Internal axis | Review axis |
|---|---|
| `novelty-significance` | `originality` or `clinical-educational-significance` (choose by the challenged claim) |
| `mechanism-evidence`, `experimental-design`, `statistical-rigor`, `clinical-validity`, `causal-vs-correlative` | `methodological-rigour` |
| `reproducibility`, `ethical-governance`, `data-resource-quality` | `ethics-reporting-transparency` |
| `figures-and-tables`, `writing-clarity` | `interdisciplinary-clarity` |
| `claim-moderation` | `clinical-educational-significance` or `methodological-rigour` (choose by whether the overreach is about impact or about inference) |
...
- Do not expose specialist personas such as `Statistics Reviewer` or `Qualitative Methodologist`, and do not infer reviewer-selection history.
