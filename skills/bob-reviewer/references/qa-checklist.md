...
- Every substantive evaluation should be traceable to either:
  - the public review principles summarised in `references/source-basis.md` (ICMJE, COPE, EQUATOR), or
...
- The technical taxonomy supplements the five review axes and does not create policy claims or severity statistics.
...
- Confirm a `中文核對` block exists, is in Traditional Chinese (Taiwan), contains exactly three committee questions (or fewer with an explicit reason), and adds no concern absent from the frozen reports.
- Confirm a `Risk / unsupported claims` section exists.
- Confirm `Review setup` names the study design and the applicable reporting guideline, or states that the guideline is undetermined.
...
- Confirm fit-for-journal judgment is expressed cautiously, because that judgment belongs to the editor.
- Confirm no specific journal's word, table, or reference limit is stated from memory.
- Confirm the output did not silently take over the role of `academic-peer-reviewer`, `equator-guideline-finder`, `bob-statistics`, or `bob-response`.
...
- The `中文核對` block uses full-width Chinese punctuation, no Simplified Chinese characters, and no mainland-China terminology.
...
- Methodological or reporting failings listed in the synthesis should match issues already raised in at least one individual report.
