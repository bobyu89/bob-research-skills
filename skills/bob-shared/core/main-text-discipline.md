...
- [5. Separate main text, captions, and Supplementary](#5-separate-main-text-captions-and-supplementary)
...
Methods, source data, and the appendix or supplementary material (hereafter "Supplementary"; 附錄／補充資料). Do not force that full
...
| `qualification` | Does it materially bound or alter the central interpretation? | Main text if yes; otherwise Supplementary |
| `robustness` | Does it show the result survives an alternative specification, estimator, seed, threshold, or inference procedure without changing the conclusion? | Supplementary, with a concise pointer when useful |
| `heterogeneity` | Is variation across groups, settings, tasks, or models itself part of the central claim? | Main text if central; otherwise Supplementary |
| `provenance_detail` | Does it document traceability, preprocessing, implementation, or audit detail without advancing the conclusion? | Methods, Source Data, repository, or Supplementary |
| `alternative_inference` | Does it test the same claim using a secondary inferential route? | Supplementary unless it changes acceptance of the claim |
| `edge_case` | Does it define a failure boundary that changes how the claim must be read? | Main text if interpretation changes; otherwise Supplementary |
...
to Supplementary with stable pointers. Draft the final Results narrative only after the
...
Keep the first in the main text when necessary. Route the second to Supplementary or the
...
## 5. Separate main text, captions, and Supplementary
...
- **Supplementary (附錄／補充資料):** why the conclusion survives deeper scrutiny, including secondary
...
multiplicity checks, sensitivity analyses, and model-level heterogeneity to Supplementary
...
- **No:** keep it.
- **Yes, but a reviewer might ask for it:** route it to Supplementary or the response letter.
...
reconciliation to Supplementary. Keep the longer explanation in the main text only when the
...
1. **Result-allocation table:** result, class, effect on central interpretation,
   destination, and Supplementary/caption pointer.
...
3. **Deletion log:** appended, replaced, compressed, relocated, or deleted text,
   with a short reason.
4. **Statistics-location record:** primary main-text report and secondary Supplementary
   analyses.
...
- Do not move information required for reproducibility, research integrity,
  ...
  words.
- Do not bury contradictory or conclusion-changing evidence in Supplementary.
