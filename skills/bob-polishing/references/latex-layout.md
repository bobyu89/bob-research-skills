# LaTeX layout & float typesetting for journal manuscripts

> Optional path. Most of Bob's deliverables are Word files; for those load
> `references/word-layout.md` instead. Use this file only when the manuscript
> or supplementary material is actually built with LaTeX.
...
- [3. Avoid rotating to landscape (most journal house styles)](#3-avoid-rotating-to-landscape-most-journal-house-styles)
...
## 3. Avoid rotating to landscape (most journal house styles)

Most journals discourage rotated/landscape figures that force the reader to
turn the page. **Prefer a taller redraw (§2) over `\rotatebox` /
`sidewaysfigure`.** Landscape is *tolerated* in SI for genuinely un-squarable figures,
but try the redraw first. If the user defers, choose upright.
